...
FETS.getSurfaceExposedToSun = function() {
	...
	const htm =
		`
			<details ontoggle="FXSTIdivSurfaceType.innerHTML=FETS.getSurfaceExposedToSun();" >
...
FETS.getSurfaceExposedToSun = function() {
	...
	errors = FETS.errors.map( item => `id: ${ item.id } surface type: ${ item.typeSource } - exposedToSun current value: ${ ( item.exposedToSun && item.exposedToSun.length ? item.exposedToSun[ 1 ] : "missing attribite" ) }` );
