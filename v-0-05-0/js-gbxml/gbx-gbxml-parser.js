...
GBX.parseFile = function( gbxml )  {
	...
	GBX.openings = GBX.gbxml.match( /<Opening(.*?)<\/Opening>/gis );

	if ( GBX.openings ) {

		GBX.openings.forEach( opening => {
			...
			if ( !idRef ) {
				...
				GBX.gbxml = GBX.gbxml.replace( opening, newOpening );
	...
	GBX.surfaces = GBX.gbxml.match( /<Surface(.*?)<\/Surface>/gis );
	...
	GBX.surfaces.forEach( surface => {
			...
			if ( !construction ) {
				...
				GBX.gbxml = GBX.gbxml.replace( surface, newSurface );
	...
	GBX.gbxml = GBX.gbxml.replace( /<\/Campus>/i, `</Campus> ${ constructions2 }` )

	divContents.innerHTML =
	`<p>${ GBX.gbxml.length } bytes</p>
	<p>Ready to save</p>
	`;

	FSB.sourceContents = GBX.gbxml;
